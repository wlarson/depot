...
class ProductTest < ActiveSupport::TestCase
  ...
  test "that the price must be at least one cent" do
    ...
    product.price = 0.001
    assert !product.valid?
    assert product.errors.on(:price), "price must be at least one cent"

    product.price = nil
    assert !product.valid?
    assert product.errors.on(:price), "nil price should be invalid"

    product.price = 1
    assert product.save!, "product.save! failed"
  end
